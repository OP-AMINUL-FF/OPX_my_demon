...
static void _notifyPWM(PWMState *p, bool idle) {
  p->pwmUpdate = nullptr;
...
bool _stopPWM_weak(uint8_t pin) {
...
int stopWaveform_weak(uint8_t pin) {
...
// other version.
static inline uint32_t GetCycleCountIRQ() {
  uint32_t ccount;
...
static inline uint32_t earliest(uint32_t a, uint32_t b) {
